...
%define etc_accumulo /etc/%{name}
%define accumulo_home /usr/lib/%{name}
%define bin_accumulo %{accumulo_home}/bin
%define lib_accumulo %{accumulo_home}/lib
%define conf_accumulo %{accumulo_home}/conf
%define logs_accumulo %{accumulo_home}/logs
%define pids_accumulo %{accumulo_home}/pids
%define webapps_accumulo %{accumulo_home}/accumulo-webapps
%define man_dir %{_mandir}
%define accumulo_username accumulo
%define accumulo_services master regionserver thrift
%define hadoop_home /usr/lib/hadoop
%define zookeeper_home /usr/lib/zookeeper
%define accumulo_jar_deps %{hadoop_home}/hadoop-core.jar %{zookeeper_home}/zookeeper.jar
...
License: APL2
Source0: %{name}-%{accumulo_base_version}.tar.gz
Source1: do-component-build
Source2: install_accumulo.sh
Source3: accumulo.sh
Source4: accumulo.sh.suse
Source5: accumulo.default
Source6: accumulo.nofiles.conf
BuildArch: noarch
...
ln -s %{_localstatedir}/log/%{name} %{buildroot}/%{logs_accumulo}

%__install -d  -m 0755  %{buildroot}/%{_localstatedir}/run/%{name}
ln -s %{_localstatedir}/run/%{name} %{buildroot}/%{pids_accumulo}
...
for service in %{accumulo_services}
do
	init_file=$RPM_BUILD_ROOT/%{initd_dir}/%{name}-${service}
	%__cp $orig_init_file $init_file
	%__sed -i -e "s|@ACCUMULO_DAEMON@|${service}|" $init_file
	chmod 755 $init_file
done
...
ln -f -s %{accumulo_jar_deps} $RPM_BUILD_ROOT/%{lib_accumulo}

%pre
getent group accumulo 2>/dev/null >/dev/null || /usr/sbin/groupadd -r accumulo
getent passwd accumulo 2>&1 > /dev/null || /usr/sbin/useradd -c "HBase" -s /sbin/nologin -g accumulo -r -d /var/run/accumulo accumulo 2> /dev/null || :
...
%{logs_accumulo}
%{pids_accumulo}
%dir %{_localstatedir}/log/accumulo
%dir %{_localstatedir}/run/accumulo

%defattr(-,root,root)
%config(noreplace) %{_sysconfdir}/default/accumulo
%config(noreplace) /etc/security/limits.d/accumulo.nofiles.conf
%{accumulo_home}
%{accumulo_home}/accumulo-*.jar
%{webapps_accumulo}
/usr/bin/accumulo
%config(noreplace) %{etc_accumulo}/conf
